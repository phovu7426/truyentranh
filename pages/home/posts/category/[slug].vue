...
// Hàm fetch category by slug - sử dụng API mới
const fetchCategoryBySlug = async (slug) => {
  try {
    const response = await apiClient.get(publicEndpoints.postCategories.showBySlug(slug))
...
onMounted(async () => {
  ...
  if (category) {
    ...
    useSeo({
      title: category.name,
      description: category.description || `Tất cả bài viết trong danh mục ${category.name}. Khám phá các bài viết mới nhất và phổ biến nhất.`,
      type: 'website',
      url: `/home/posts/category/${category.slug || categorySlug}`
    })
  } else {
    // Fallback SEO
    useSeo({
      title: 'Danh mục bài viết',
      description: 'Khám phá các bài viết theo danh mục. Tìm kiếm và đọc các bài viết mới nhất.',
      type: 'website',
      url: `/home/posts/category/${categorySlug}`
    })
  }
